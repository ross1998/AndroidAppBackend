...
router.post('/login', (req, res) => {
    ...
    if(email && theirPw) {
        ...
        db.one('SELECT Pword, Salt FROM Profiles WHERE Email=$1', [email])
            .then(row => { //If successful, run function passed into .then()
                let salt = row['salt'];
                //Retrieve our copy of the password
                let ourSaltedHash = row['pword'];
...
router.post("/getprofile", (req, res) => {
    ...
    let email = req.body['email'];
        ...
        if(email){
            db.manyOrNone('SELECT fname, height, bodyweight, gender, age FROM profiles WHERE Email=$1', [email])
